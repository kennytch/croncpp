...
namespace cron
{
   ...
   namespace utils
   {
      ...
      inline std::time_t timegm(std::tm const* t)     
      {
         ...
         int days_since_epoch = utils::days_from_epoch(year, month + 1, t->tm_mday);
      ...
      {
         std::time_t rec = utils::timegm(&date);
         // implicit change to date
         date = *std::gmtime(&rec);
         return rec;
      }
